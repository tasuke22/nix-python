...
### Q. devShell の中にいると何が嬉しいの？

プロジェクトごとに開発環境を固定・共有できる。

- **バージョン固定** — `flake.lock` がある限り、半年後でも同じバージョンのツールが使える
- **自動セットアップ** — `cd` するだけで必要なツールが全部揃う。手順書が不要
- **プロジェクト間の隔離** — 別プロジェクトで ruff 0.12 を使っていても、このプロジェクトでは 0.15 が使える
- **チーム共有** — `flake.nix` + `flake.lock` をコミットすれば、他の人も同じ環境を再現できる

逆に、自分一人でバージョン固定も不要なら、devShell のメリットは薄い。

### Q. `nix develop` と `direnv allow` の違いは？

同じもの。`.envrc` の `use flake` が裏で `nix develop` 相当のことをやっている。

- **普段は `direnv allow` だけでOK**。一度実行すれば、以降は `cd` するだけで自動
- `nix develop` を直接使うのは、direnv がない環境や CI くらい

### Q. Nix と .venv はどういう関係？両方あるの？
...
Nix はシステムレベルの開発ツールを提供する外側の箱。.venv は Python ライブラリを入れる内側の箱。

Nix だけで Python ライブラリも管理する方法はあるが、PyPI のパッケージは Nix のパッケージ形式と合わないものが多く面倒。なので実用的には：

- **CLI ツール** → Nix で管理（バージョン固定、チーム共有が楽）
- **Python ライブラリ** → uv + .venv で管理（PyPI そのまま使えて楽）
